...
if(isset($_POST["register"]))
{
	...
	if($error_user_name == '' && $error_user_email == '' && $error_user_password == '' && $error_user_cpassword == '')
	{
		...
		$query = "
		INSERT INTO admins
		(adm_name, adm_email, adm_password, adm_token, adm_status, adm_otp)
		SELECT * FROM (SELECT :user_name, :user_email, :user_password, :user_activation_code, :user_email_status, :user_otp) AS tmp
		WHERE NOT EXISTS (
		    SELECT adm_email FROM admins WHERE adm_email = :user_email
		) LIMIT 1
		";
		...
		else
		{
			...
			if($mail->Send())
			{
				...
				header('location:adm_email_verify.php?code='.$user_activation_code);
			}
			else
			{
...
<div class = "logo">
    ...
    <br />
        ...
        <img src="SFLogo.png" class="logo" width = "210" height = "105" style = "display: block; margin-left:auto; margin-right:auto; " >
			<h1 align="center">SIGN UP AS AN ADMIN ON SMARTFARM</h1>
			<br />
